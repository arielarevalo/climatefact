...
La calidad del subsistema de recuperación se evalúa mediante métricas estándar de sistemas de recuperación de información, aplicadas para diferentes valores de \textit{k} (número de documentos recuperados):

\begin{itemize}
\item \textbf{Recall@k}: Fracción de evidencia relevante recuperada entre los primeros \textit{k} resultados.
\item \textbf{Precision@k}: Fracción de documentos relevantes entre los \textit{k} documentos recuperados.
...
Estas métricas se calculan de forma independiente para cada estrategia de recuperación (híbrida conceptual, semántica por embeddings, y combinación fusionada) con valores de \textit{k} $\in$ \{1, 3, 5, 10\}, permitiendo evaluar tanto la precisión como la exhaustividad del proceso de recuperación.
