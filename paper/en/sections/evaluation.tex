...
The quality of the retrieval subsystem is evaluated using standard information retrieval metrics, applied for different values of \textit{k} (number of retrieved documents):

\begin{itemize}
\item \textbf{Recall@k}: Fraction of relevant evidence retrieved among the top \textit{k} results.
\item \textbf{Precision@k}: Fraction of relevant documents among the \textit{k} retrieved documents.
...
These metrics are calculated independently for each retrieval strategy (conceptual hybrid, semantic embedding, and fused combination) with values of \textit{k} $\in$ \{1, 3, 5, 10\}, allowing evaluation of both the precision and exhaustiveness of the retrieval process.
