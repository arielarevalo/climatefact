...
class RetrievalEvaluator:
    ...
    def __init__(self, metrics_calculator: Optional[RetrievalMetrics] = None):
    ...
    def create_evaluation_pairs(self, 
                              gold_entries: List[Dict[str, Any]], 
                              retrieved_results: List[List[Dict[str, Any]]]) -> List[Tuple[List[Dict[str, Any]], str]]:
        """
        ...
        for gold_entry, retrieved_passages in zip(gold_entries, retrieved_results):
    ...
    def generate_detailed_report(self, 
        ...
        if method_results:
            ...
            report["evaluation_summary"]["k_values"] = [
                int(k.replace("k_", "")) for k in first_method.keys()
            ]
    ...
    def _generate_recommendations(self, method_results: Dict[str, Dict[str, Dict[str, float]]]) -> List[str]:
        ...
        if win_percentages:
            ...
            recommendations.append(
                f"Overall best performing method: {best_overall[0]} "
                f"(wins {best_overall[1]:.1f}% of comparisons)"
            )
        ...
        if "k_5" in best_methods:  # Focus on k=5 as a good middle ground
            ...
            if "recall" in k5_results:
                recommendations.append(
                    f"Best recall@5: {k5_results['recall']['method']} "
                    f"({k5_results['recall']['score']:.4f})"
                )
            
            if "precision" in k5_results:
                recommendations.append(
                    f"Best precision@5: {k5_results['precision']['method']} "
                    f"({k5_results['precision']['score']:.4f})"
                )
            
            if "ndcg" in k5_results:
                recommendations.append(
                    f"Best ranking quality (nDCG@5): {k5_results['ndcg']['method']} "
                    f"({k5_results['ndcg']['score']:.4f})"
                )
        ...
        for method_name, results in method_results.items():
            if "k_5" in results:
                ...
                if recall_5 < 0.3:
                    recommendations.append(
                        f"Warning: {method_name} has low recall@5 ({recall_5:.3f}). "
                        "Consider improving coverage."
                    )
