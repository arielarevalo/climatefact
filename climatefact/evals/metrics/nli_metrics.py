...
class NLIMetrics:
    ...
    def normalize_label(self, label: str) -> str:
        ...
        if label in ['entailment', 'entail', 'supports', 'true']:
            ...
            raise ValueError(f"Unknown label '{label}' - expected one of: entailment, contradiction, neutral (or their variations)")
    ...
    def calculate_accuracy(self, predicted_labels: List[str], true_labels: List[str]) -> float:
        ...
        correct = sum(1 for p, t in zip(pred_normalized, true_normalized) if p == t)
        accuracy = correct / len(predicted_labels)
        
        return accuracy
    
    def calculate_precision_recall_f1(self, predicted_labels: List[str], true_labels: List[str], 
        ...
        # Normalize labels
        pred_normalized = [self.normalize_label(l) for l in predicted_labels]
        true_normalized = [self.normalize_label(l) for l in true_labels]
        target_label = self.normalize_label(label)
        
        # Calculate TP, FP, FN
        tp = sum(1 for p, t in zip(pred_normalized, true_normalized) 
                if p == target_label and t == target_label)
        fp = sum(1 for p, t in zip(pred_normalized, true_normalized) 
                if p == target_label and t != target_label)
        fn = sum(1 for p, t in zip(pred_normalized, true_normalized) 
                if p != target_label and t == target_label)
    ...
    def calculate_confusion_matrix(self, predicted_labels: List[str], 
                                 true_labels: List[str]) -> Dict[str, Dict[str, int]]:
        """
        ...
        for pred, true in zip(pred_normalized, true_normalized):
            confusion_matrix[true][pred] += 1
        
        return confusion_matrix
    
    def calculate_macro_averages(self, predicted_labels: List[str], 
    ...
    def evaluate_single_prediction(self, predicted_label: str, true_label: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'correct': pred_normalized == true_normalized
        }
    
    def evaluate_multiple_predictions(self, 
                                    predictions: List[Tuple[str, str]]) -> Dict[str, Any]:
        """
        ...
        if confusion:
            ...
            for true_label in ['ENTAILMENT', 'CONTRADICTION', 'NEUTRAL']:
                if true_label in confusion:
                    ...
                    summary += f"\n{true_label[:3]}\t\t{row.get('ENTAILMENT', 0)}\t{row.get('CONTRADICTION', 0)}\t{row.get('NEUTRAL', 0)}"
