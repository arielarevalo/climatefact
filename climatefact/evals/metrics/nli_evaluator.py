...
class NLIEvaluator:
    ...
    def __init__(self, metrics_calculator: Optional[NLIMetrics] = None):
    ...
    def load_passages(self, passages_path: str) -> Dict[str, str]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load passages from {passages_path}: {e}")
    ...
    def load_gold_set(self, gold_set_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load gold set from {gold_set_path}: {e}")
    ...
    def run_nli_evaluation(self, test_cases: List[Tuple[str, str, str]]) -> List[Tuple[str, str]]:
        ...
        for i, (claim, passage_text, expected_label) in enumerate(test_cases):
            ...
            try:
                ...
                raise RuntimeError(f"NLI model failed on test case {i+1} (claim: '{claim[:50]}...', passage: '{passage_text[:50]}...'): {e}")
    ...
    def evaluate_nli_performance(self, 
                                gold_set_path: str,
                                passages_path: str,
        ...
        # Load data
        passages = self.load_passages(passages_path)
