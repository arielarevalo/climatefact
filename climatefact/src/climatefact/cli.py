...
def main():
    ...
    parser.add_argument(
        "statement",
        help="Climate statement to analyze for contradictions"
    )
    
    parser.add_argument(
        "--passages-path",
        default="../data/passages.jsonl",
        help="Path to the passages JSONL file"
    )
    
    parser.add_argument(
        "--concept-index-path",
        default="../data/concept_index.json",
        help="Path to the concept index JSON file"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of top passages to retrieve for hybrid retrieval"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of top passages to retrieve for semantic search"
    )
    ...
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Enable verbose output with raw results"
    )
    ...
    try:
        ...
        print(f"❌ Error occurred during analysis: {str(e)}", file=sys.stderr)
        if args.verbose:
            import traceback
            traceback.print_exc()
        sys.exit(1)
