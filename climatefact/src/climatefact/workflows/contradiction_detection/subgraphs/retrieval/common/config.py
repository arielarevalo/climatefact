...
REGEX_MAP: dict[str, Pattern[str]] = {
    ...
    "co2_ffi": re.compile(r"\b(?:CO[₂2][-–]FFI|CO[₂2]\s+from\s+fossil\s+fuels\s+and\s+industry)\b", re.I),  # noqa: RUF001
    ...
    "imp_ren": re.compile(r"\bIMP\s*[-–]?\s*REN\b", re.I),  # noqa: RUF001
    ...
    "imp_ld": re.compile(r"\bIMP\s*[-–]?\s*LD\b", re.I),  # noqa: RUF001
    "imp_neg": re.compile(r"\bIMP\s*[-–]?\s*NEG\b", re.I),  # noqa: RUF001
    "ip_modact": re.compile(r"\bIP[-–]?ModAct\b", re.I),  # noqa: RUF001
