    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_task):
            ...
            try:
                ...
                logger.debug(
                    f"Completed NLI task for sentence {result.sentence_idx}, passage {result.passage_idx}: {result.label}"
                )
            except Exception as e:
                logger.error(
...
def detect_contradictions(state: ContradictionDetectionState) -> ContradictionDetectionState:
    ...
    if len(queries) != len(retrieved_passages_for_sentences):
        logger.warning(
            f"Mismatch between number of queries ({len(queries)}) and retrieved passage sets ({len(retrieved_passages_for_sentences)})."
        )
    ...
    logger.info(
        f"Found {total_contradictions} total contradictions across {sentences_with_contradictions} sentences using concurrent processing."
    )
