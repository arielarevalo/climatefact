...
from langchain_community.llms.azureml_endpoint import AzureMLEndpointApiType, AzureMLOnlineEndpoint, ContentFormatterBase
from langchain_core.outputs import Generation
from pydantic import SecretStr
from climatefact.workflows.contradiction_detection.types import ContradictionDetectionState, ContradictionResult, ContradictionEvidence
...
class NLIContentFormatter(ContentFormatterBase):
    ...
    def format_request_payload(self, prompt: str, model_kwargs: Dict, api_type: AzureMLEndpointApiType = AzureMLEndpointApiType.dedicated) -> bytes:
...
def validate_azure_nli_config() -> Dict[str, str]:
    ...
    if not endpoint or not api_key:
        ...
        logger.error(
            f"Missing required Azure ML NLI environment variables: {', '.join(missing_vars)}"
        )
        return {}
...
_nli_client: Optional[AzureMLOnlineEndpoint] = None


def get_nli_client() -> Optional[AzureMLOnlineEndpoint]:
    ...
    try:
        ...
        logger.info(
            f"Successfully initialized Azure ML NLI client with endpoint: {config['endpoint']}"
        )
        return _nli_client
...
def _call_nli_model_sync(client: AzureMLOnlineEndpoint, premise: str, hypothesis: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error calling NLI model for premise '{premise[:30]}...' and hypothesis '{hypothesis[:30]}...': {e}")
...
def _prepare_nli_tasks(queries: List[str], retrieved_passages_for_sentences: List[List[Dict[str, Any]]]) -> List[NLITask]:
...
def _process_nli_tasks_concurrently(client: AzureMLOnlineEndpoint, tasks: List[NLITask], max_workers: int = 10) -> List[NLIResult]:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_task = {
            executor.submit(_process_nli_task, client, task): task 
            for task in tasks
        }
        ...
        for future in as_completed(future_to_task):
            ...
            try:
                ...
                logger.debug(f"Completed NLI task for sentence {result.sentence_idx}, passage {result.passage_idx}: {result.label}")
            except Exception as e:
                logger.error(f"Error processing NLI task for sentence {task.sentence_idx}, passage {task.passage_idx}: {e}")
            ...
            if result.label == "contradiction":
                sentence_contradictions.append(
                    ContradictionEvidence(
                        contradictory_passage=result.passage_text,
                        source=result.passage_source
                    )
                )
...
def detect_contradictions(state: ContradictionDetectionState) -> ContradictionDetectionState:
    ...
    for i, passage_set in enumerate(retrieved_passages_for_sentences):
        ...
        if passage_set:
            for j, passage in enumerate(passage_set[:2]):  # Log first 2 passages
                logger.debug(f"  Passage {j}: {passage.get('text', '')[:100]}... from {passage.get('source', 'Unknown')}")
    ...
    if len(queries) != len(retrieved_passages_for_sentences):
        logger.warning(f"Mismatch between number of queries ({len(queries)}) and retrieved passage sets ({len(retrieved_passages_for_sentences)}).")
    ...
    logger.info(f"Found {total_contradictions} total contradictions across {sentences_with_contradictions} sentences using concurrent processing.")
