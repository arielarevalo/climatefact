...
def create_sample_state() -> ContradictionDetectionState:
    ...
    return cast(ContradictionDetectionState, {
        ...
        "retrieved_data_for_queries": [
            [  # Passages for first query
                {"text": "Climate change is extensively documented by scientific research", "source": "IPCC Report 2023"},
            ...
            [  # Passages for second query
                {"text": "Global mean temperature has increased by 1.1°C since pre-industrial times", "source": "WMO State of Climate 2023"},
                {"text": "The last decade was the warmest on record", "source": "Berkeley Earth Temperature Study"}
            ],
            [  # Passages for third query
                {"text": "Sea levels have risen approximately 20cm since 1900", "source": "Coastal Research Institute"},
                {"text": "Ice sheet melting contributes to accelerating sea level rise", "source": "Glaciology Research Center"},
                {"text": "Thermal expansion of oceans drives sea level increase", "source": "Oceanographic Institute"}
            ]
        ],
...
def demonstrate_concurrent_processing():
    ...
    print(f"\nTotal NLI comparisons to perform: {sum(len(passages) for passages in (state['retrieved_data_for_queries'] or []))}")
